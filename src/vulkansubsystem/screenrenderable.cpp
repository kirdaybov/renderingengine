...
{
  ShaderBinary* vsShader = gApp.GetShaderCompiler().GetShader("screenvs");
  ShaderBinary* psShader = gApp.GetShaderCompiler().GetShader("screenps");
...
void ScreenRenderable::CreateVertexBuffer()
{
...
void ScreenRenderable::LoadModelFBX()
{
  m_Vertices.push_back(Vertex({-1,-1,0}));
  m_Vertices.push_back(Vertex({ -1,1,0 }));
  m_Vertices.push_back(Vertex({ 1,1,0 }));
  m_Vertices.push_back(Vertex({ 1,-1,0 }));
...
void ScreenRenderable::Update(RenderContext& ctx)
{
  UpdateUniformBuffer();
}

void ScreenRenderable::Render(RenderContext& ctx)
{
...
void ScreenRenderable::OnCleanupSwapChain()
{
